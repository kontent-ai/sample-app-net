...
namespace DancingGoat.InlineContentItemResolver
{
    public class HostedVideoResolver : IInlineContentItemsResolver<HostedVideo>
    {
        public string Resolve(ResolvedContentItemData<HostedVideo> data)
        {
            ...
            if (selected == "Vimeo")
                return
                    $"<div><iframe src=\"https://player.vimeo.com/video/{data.Item.VideoId}?title=0&byline=0&portrait=0\" width=\"640\" " +
                    $"height=\"360\" frameborder=\"0\" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe><p></div>";
            if (selected == "Youtube")
            {
                return $"<iframe width=\"560\" height=\"315\" class=\"hosted-video__wrapper\" " +
